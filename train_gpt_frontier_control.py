...
class Muon(torch.optim.Optimizer):
    ...
    def export_runtime_state(self) -> dict[str, object]:
        ...
        return {
            ...
            "shard_momentum": [m["shard_mom"].detach().cpu() for m in self._bank_meta],
        }
    ...
    def load_runtime_state(self, state: dict[str, object]) -> None:
        ...
        if not self._built:
            self._build()
        expected_world_size = int(state.get("world_size", self._world_size))
        if expected_world_size != self._world_size:
            raise ValueError(
                f"Muon runtime checkpoint expects WORLD_SIZE={expected_world_size}, got {self._world_size}"
            )
        shard_momentum = state.get("shard_momentum") or []
        if len(shard_momentum) != len(self._bank_meta):
            raise ValueError(
                f"Muon runtime checkpoint has {len(shard_momentum)} shard buffers, expected {len(self._bank_meta)}"
            )
        for saved, meta in zip(shard_momentum, self._bank_meta, strict=True):
            meta["shard_mom"].copy_(saved.to(device=meta["shard_mom"].device, dtype=meta["shard_mom"].dtype))
...
def main() -> None:
        ...
        atomic_torch_save(
            {
                ...
                "optimizer_muon_runtime": optimizer_muon.export_runtime_state(),
                "train_loader_state": train_loader.state_dict(),
                "swa_state": _state_dict_to_cpu(swa_state),
                "swa_count": swa_count,
                "lawa_queue": [{name: tensor.detach().cpu() for name, tensor in snap.items()} for snap in lawa_queue],
    ...
    if args.resume_from:
        ...
        optimizer_muon.load_runtime_state(checkpoint.get("optimizer_muon_runtime") or {})
        ...
        lawa_queue = deque(
            [{name: tensor.to(device=device) for name, tensor in snap.items()} for snap in checkpoint.get("lawa_queue", [])],
            maxlen=args.lawa_k,
        )
    ...
    if args.eval_stride > 0 and args.eval_stride < sw_seq_len:
        ...
        final_exact_evals["final_int8_zlib_roundtrip"] = {"val_loss": sw_val_loss, "val_bpb": sw_val_bpb}
    if args.eval_stride != 64 and 64 < sw_seq_len:
        ...
        final_exact_evals["final_int6_sliding_window_s64"] = {"val_loss": sw64_val_loss, "val_bpb": sw64_val_bpb}
        final_exact_evals["final_int8_zlib_roundtrip"] = {"val_loss": sw64_val_loss, "val_bpb": sw64_val_bpb}
    ...
    if args.ttt_enabled:
        ...
        final_exact_evals["legal_ttt"] = {"val_loss": ttt_loss, "val_bpb": ttt_bpb}
